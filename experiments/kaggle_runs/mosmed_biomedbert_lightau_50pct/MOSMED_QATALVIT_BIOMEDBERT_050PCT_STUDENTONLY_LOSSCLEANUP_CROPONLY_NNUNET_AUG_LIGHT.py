...
class CFG:
    ...
    final_test_threshold: float = 0.55
...
# Test is evaluated once using a fixed operating threshold for the submitted run.
test_thr = float(getattr(cfg, "final_test_threshold", 0.55))
test_metrics = evaluate(best_model, test_loader, device=device, threshold=test_thr, criterion=criterion)
...
if 'test_metrics' in globals():
    ...
    if eval_model is not None and 'test_loader' in globals():
        tm = evaluate(eval_model, test_loader, device=device, threshold=float(globals().get('test_thr', getattr(cfg, 'final_test_threshold', 0.55))), criterion=criterion)
    else:
        tm = {}
...
if eval_model is not None and 'unlabeled_ds' in globals():
    pseudo_report = _pseudo_holdout_report(eval_model, unlabeled_ds, threshold=float(globals().get('test_thr', getattr(cfg, 'final_test_threshold', 0.55))))
else:
...
saved_vis = _export_visualize_val(
    ...
    visualize_dir,
    threshold=float(globals().get('test_thr', getattr(cfg, 'final_test_threshold', 0.55))),
    max_items=48,
)
